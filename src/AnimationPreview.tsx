...
export default function AnimationPreview({ selectedCurve, duration }: AnimationPreviewProps) {
  ...
  return (
    <div
      className="w-full h-full bg-[#3C3C3C] rounded-lg flex items-center justify-center"
      aria-label="Animation Preview"
    >
      <motion.div className="w-16 h-16 bg-[#0D99FF] rounded" animate={controls} initial={{ y: 50 }} />
    </div>
  );
}
